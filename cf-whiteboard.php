...
function cfw_admin_notice() {
    ...
    if (current_user_can('manage_options')) {
        ...
        if (!get_user_meta($user_id, 'cfw_getstarted_notice')) {
            ...
            if (!get_user_meta($user_id, 'cfw_installation_notice')) {
                $pointer_content .= 'Thank you for installing <a href="http://cfwhiteboard.com" target="_blank" style="color:#01a1ff; text-decoration:underline;">CF Whiteboard</a>! Please visit <a href="options-general.php?page=cf-whiteboard.php" style="color:#01a1ff; text-decoration:underline;">Settings > CF Whiteboard > My Account</a> to activate your free trial and get started.';
            } else {
                // beta customer
                $pointer_content .= '<strong style="display:block; font-size:19px; font-weight:bold; line-height:26px;">It\'s time to sign up!</strong> Thank you for being one of <a href="http://cfwhiteboard.com" target="_blank" style="color:#01a1ff; text-decoration:underline;">CF Whiteboard</a>\'s first customers. We now have over 50 gyms using our service, so we\'re transitioning out of the beta testing phase. You will receive a one month free trial before you are required to sign up. Click <strong>Sign Up Now</strong> for more details.';
                $pointer_content .= '</p>';
                $pointer_content .= '<p style="margin:0; padding:0 0 12px;">';
                $pointer_content .= '<strong style="display:block; font-size:19px; font-weight:bold; line-height:26px;">We\'re not done yet.</strong> Right now we\'re working on advanced PR/Benchmark tracking to give you insight into your gym\'s performance over time, and to give athletes instant access to all of their PR data. <a href="mailto:affiliatesupport@cfwhiteboard.com" style="color:#01a1ff; text-decoration:underline;">Email us</a> anytime you want to chat about what we\'re working on or what you want us to work on!';
            }
            ...
            if (!get_user_meta($user_id, 'cfw_installation_notice')) {
                ...
                $pointer_content .= '<a href="options-general.php?page=cf-whiteboard.php#account" class="button-primary">Sign Up Now</a>';
            }
            $pointer_content .= '<a href="'.$current_url.'&cfw_ignore=cfw_getstarted_notice" class="button" style="margin-left:10px;">Destroy this message</a>';
            $pointer_content .= '</p>';
            $pointer_content .= '</div>';
            $pointer_content .= '</div>';
...
function cfw_notice_ignore() {
    ...
    if ( isset($_GET['cfw_ignore']) && 'cfw_getstarted_notice' == $_GET['cfw_ignore'] ) {
         add_user_meta($user_id, 'cfw_getstarted_notice', 'true', true);
    }
}
            ...
            <form method="post" action="" style="max-width:600px;">
                ...
                <div class="navbar js-scroll-here-on-iframe-reload">
                    <div class="navbar-inner" style="padding-right:0;">
                        <ul class="nav">
                            <li class="dropdown">
                                <a href="javascript://" class="dropdown-toggle brand" data-toggle="dropdown" style="background:url(<?php echo plugins_url('images/cfwhiteboard.png', __FILE__); ?>) no-repeat 5px 4px; text-indent:140px;">
                                    <b class="caret"></b>
                                </a>
                ...
                <div class="tab-content" style="overflow:visible;">
                    <div id="welcome" class="tab-pane active">
                        ...
                        <p>
                            Please <a href="mailto:affiliatesupport@cfwhiteboard.com">email us</a> with any questions.
 ...
 -->
                ...
                <div class="form-actions">
                    ...
                    <span class="help-inline pull-right">
                        ...
                        <a href="mailto:affiliatesupport@cfwhiteboard.com" class="btn btn-link"><?php _e('Email Us', 'cf-whiteboard'); ?></a>
                    </span>
                </div>
...
function cfwhiteboard_generate_class_fields($class) {
            ...
            <tr>
                <th>
                    <label for="'. $class_name_attr .'">
                        ...
                        <a href="javascript://" class="help-alert" data-message="Each class gets a separate tab in the Whiteboard. You can track as many classes as you want.<br><br>Example Class Names: WOD, Oly, Strength, Endurance, Erg, Mobility">(Help)</a>
                    </label>
                </th>
                <td>
    ...
    <div class="tools cfw-twb">
        <a href="javascript://" class="add-component btn btn-small btn-inverse" style="margin:5px 6px 6px;" data-classid="'. $class_id .'">
...
function cfwhiteboard_wods_meta_box($object, $box) {
    ...
    if (!empty($plugin_updates) && !empty($plugin_updates->response) && !empty($plugin_updates->response["cf-whiteboard/cf-whiteboard.php"])) {
        ...
        ?>
            <div class="cfw-twb">
                <div class="alert alert-info" style="color:#505050; margin:0 6px 20px; padding-bottom:6px;">
                    <!-- <button type="button" class="close" data-dismiss="alert">×</button> -->
                    <strong>New Features!</strong>
                    A new version of CF Whiteboard is ready.
                    <span class="nobr">
                        <a class="btn btn-primary" style="margin:0 2px 2px 10px;" target="_blank" href="<?php echo wp_nonce_url( self_admin_url('update.php?action=upgrade-plugin&plugin=') . $file, 'upgrade-plugin_' . $file) ?>">
...
*/
    ...
    <div id="cfw-subscription-status-notice" style="display:none;" class="cfw-twb">
        <div class="trial_expired alert alert-info" style="color:#505050; margin:0 6px 20px; display:none;">
            <strong>Your free trial has expired.</strong>
            <a href="options-general.php?page=cf-whiteboard.php#account">Choose a subscription</a> to continue using CF Whiteboard, or <a href="mailto:affiliatesupport@cfwhiteboard.com">email us</a> with any questions.
        </div>
        <div class="past_due alert alert-info" style="color:#505050; margin:0 6px 20px; display:none;">
            <strong>Your subscription is past due.</strong>
            Visit <a href="options-general.php?page=cf-whiteboard.php#account">Settings > CF Whiteboard > My Account</a> for more information, or <a href="mailto:affiliatesupport@cfwhiteboard.com">email us</a> with any questions.
        </div>
        <div class="canceled alert alert-info" style="color:#505050; margin:0 6px 20px; display:none;">
            <strong>Your subscription is canceled.</strong>
            Visit <a href="options-general.php?page=cf-whiteboard.php#account">Settings > CF Whiteboard > My Account</a> for more information, or <a href="mailto:affiliatesupport@cfwhiteboard.com">email us</a> with any questions.
        </div>
        <div class="expired alert alert-info" style="color:#505050; margin:0 6px 20px; display:none;">
...
-->
    ...
    <p class="tools cfw-twb">
        <a href="javascript://" class="add-class btn btn-large btn-success">
            <i class="icon-plus icon-white"></i>
            <?php esc_html_e("Track Another Class", 'cf-whiteboard'); ?>
        </a>
    </p>
    <script type="text/javascript">
        typeof(jQuery) == 'function' && jQuery(function($) {
            ...
            $(window).on('affiliateDetailsReady', function() {
                if (!CFW.affiliateDetails.activated_at) {
