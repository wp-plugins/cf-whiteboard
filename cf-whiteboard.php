...
// Show a Feature Pointer when CF Whiteboard is activated for the first time
// add_action('load-plugins.php', 'cfwhiteboard_get_started_pointer');
function cfwhiteboard_get_started_pointer() {
    add_action('admin_enqueue_scripts', 'cfwhiteboard_show_get_started_pointer');
}
function cfwhiteboard_show_get_started_pointer() {
    // Using Pointers
    wp_enqueue_style( 'wp-pointer' );
    wp_enqueue_script( 'wp-pointer' );

    // Register our action
    add_action( 'admin_print_footer_scripts', 'cfwhiteboard_get_started_pointer_footer_scripts' );
}
function cfwhiteboard_get_started_pointer_footer_scripts() {
    $pointer_content = '<h3>Welcome to CF Whiteboard!</h3>';
    $pointer_content .= '<p>We&apos;re aware that WordPress themes can make the WHITEBOARD button show up in some pretty weird places on your site, or sometimes not at all. This is easy to fix! We want to make sure you see exactly what you&apos;ll get, so contact us and we&apos;ll customize it for your site for free!';
    $pointer_content .= '<p>Check us out at CrossFit Roots or CrossFit Forever Strong to get an idea of how we look.</p>';
    $pointer_content .= '<p>Also, we&apos;re in Beta... that means the plugin is FREE! Refer a gym for additional discounts after we&apos;re out of Beta!</p>';
?>
<script type="text/javascript">
//<![CDATA[
jQuery(function($) {
    $('#cf-whiteboard').pointer({
